...
{
	MemDC CMemDC(CDC::FromHandle(lpDIS->hDC));
	CFont *poldFont = CMemDC.SelectObject(GetFont());
	...
	DrawBackground(&CMemDC, &itemRect);
	...
	if( m_csBitmaps[0].hBitmap )
	{
		...
		DrawTheBitmap(&CMemDC, !sTitle.IsEmpty(), &lpDIS->rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
	}
	...
	if( m_csIcons[0].hIcon )
	{
		...
		DrawTheIcon(&CMemDC, !sTitle.IsEmpty(), &lpDIS->rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
	}	
	...
	if( sTitle.IsEmpty() == FALSE )
	{
		...
		CMemDC.DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
		...
		{
			captionRect.OffsetRect(1, 1);
			CMemDC.SetTextColor(::GetSysColor(COLOR_3DHILIGHT));
			CMemDC.DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
			captionRect.OffsetRect(-1, -1);
			CMemDC.SetTextColor(::GetSysColor(COLOR_3DSHADOW));
			CMemDC.DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
		}
		else
		{
			...
			{
				CMemDC.SetTextColor(m_crColors[SKINBTN_COLOR_FG_IN]);
			}
			else 
			{
				CMemDC.SetTextColor(m_crColors[SKINBTN_COLOR_FG_OUT]);
			}
			CMemDC.DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER);
		}
	}
