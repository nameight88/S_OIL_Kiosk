...
BOOL CADODatabase::Open(LPCTSTR lpstrConnection, LPCTSTR lpstrUserID, LPCTSTR lpstrPassword)
{
	HRESULT hr = S_OK;
	if(IsOpen()) Close();
	if(_tcscmp(lpstrConnection, _T("")) != 0) m_strConnection = lpstrConnection;
...
BOOL CADORecordset::GetFieldValue(LPCTSTR lpFieldName, long& lValue)
{
	...
	vtFld = pField->Value;
		...
		if(vtFld.vt != VT_NULL && vtFld.vt != VT_EMPTY) val = vtFld.lVal;
		lValue = val;
		return TRUE;
...
CADOCommand::CADOCommand(CADODatabase* pAdoDatabase, CString strCommandText, int nCommandType)
{
	...
	str=m_strCommandText.AllocSysString();
...
CADOParameter::CADOParameter(int nType, long lSize, int nDirection, CString strName)
{
	...
	m_strName = strName;
	str= m_strName.AllocSysString();
	m_pParameter->Name = str;
